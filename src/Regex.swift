...
public struct Regex
{
    ...
    public init(_ p:String)
    {
        ...
        do {
            nsRegex = try NSRegularExpression(pattern: pattern, options: NSRegularExpression.Options(rawValue: 0))
        } catch {
     ...
     */
    public func replaceMatches (in string:String, with replacement:String) -> (replacements:Int, string:String)
    {
